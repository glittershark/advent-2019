...
% :- func fuel_step(float) = int is det.
% fuel_step(FuelOrMass) = Fuel :-
%     Fuel = floor_to_int(FuelOrMass / 3.0) - 2.

:- func fuel_required(float) = int is det.
fuel_required(Mass) = Fuel :-
    MaybeNegative = floor_to_int(Mass / 3.0) - 2,
    (if MaybeNegative < 0
    then Fuel = 0
    else Fuel = MaybeNegative + fuel_required(float(MaybeNegative))).
